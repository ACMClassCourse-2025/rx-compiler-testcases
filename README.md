...
- **`codegen/`** contains programs that must compile and run. Each runtime fixture supplies stdin and the expected stdout for one execution.
...
The usual runtime naming pattern keeps fixtures beside their source, for example:
...
`manifest.schema.json` is the JSON Schema for version 1 of the manifest format. Each entry contains:
